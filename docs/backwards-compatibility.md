...
- **`fallow impact` records which gate produced a run.** The local store keeps
  ...
  with a run count per source, absent when the store holds no gate run, and the
  human report gains one line naming the non-zero sources. Additive optional
  under the rule above, so `ImpactReport` stays at `schema_version` 2. The
