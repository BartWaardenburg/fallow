...
- **Dead-code findings can carry a reachability `confidence[]` array**: an entry in `unused_files[]` or `unused_exports[]` on the `dead-code` and `dead-code-grouped` envelopes (and in the `dead_code` block the audit and combined envelopes embed) may carry an additive optional `confidence` array of kebab-case tokens. It records that the verdict was computed over an import graph fallow already knows is incomplete because a source file was read but did not parse cleanly, the condition the `source-parse-degraded` workspace diagnostic reports. Two values exist today. `source-parse-degraded` means this finding's own file is the one that did not parse, so the export list the "is any export of this file referenced from a reachable module" test reads may be truncated. `incomplete-import-graph` means at least one module that IS reachable from an entry point did not parse cleanly, so an import that would have credited this path may never have been seen. The second value is a RUN-level condition and deliberately does not claim that a specific degraded file imports this path: an import the parser never saw is recorded nowhere, so the link cannot be narrowed further without re-reading and re-parsing the source. It does discriminate in one direction, and soundly: when every degraded module is itself unreachable, no missing edge attributable to a degraded parse can change a reachability verdict, because the first missing edge on any entry-point path leaves from a module that is observed reachable, so no flag is emitted. The array is advisory. It never withholds, filters, reorders, downgrades, or re-severities a finding, never trims its `actions`, and never changes an exit code; gating findings on parser errors is still refused, because oxc reports recoverable errors for valid syntax newer than the parser and gating would mute real results project-wide. The array is omitted when empty, so a project that parses cleanly is byte-identical and no `schema_version` moves under the additive-field policy. Treat the value set as OPEN in the same sense as `workspace_diagnostics[].kind`: a consumer that does not recognise a token should read it as "some confidence caveat" rather than as an error. `unused_types[]` and the other issue arrays do not carry the array.
...
- **Doctor command**: `doctor` performs deterministic, local, read-only readiness checks without analysis, cache writes, telemetry, network access, dependency installation, third-party execution, or report-file writes. It checks `root`, `config`, `workspaces`, `plugins`, `type-aware`, `dependencies`, and `cache` in that stable order; the two new checks are appended, so the existing five keep their positions. Human and JSON output are supported on stdout; `--output-file` is rejected to keep diagnosis inputs immutable. The JSON envelope has `kind: "doctor"`, `schema_version: 2`, `root: "."`, aggregate `status`, summary counts, and typed `checks[]`; remediation commands carry `cwd: "."` and declare whether they are `mutating`. A safe project-relative explicit config is retained in remediation commands. For an external or unsafe-to-render config path, Doctor omits the command and instructs the caller to reuse the same private `--config` value. Pass and advisory warning outcomes exit 0. A failed required check still emits the complete envelope and exits 2. Analysis and gate flags are rejected instead of being silently ignored. `schema_version` moved to 2 for the `dependencies` and `cache` checks. Both are advisory (`required: false`) and neither can fail: `dependencies` warns when the project has no `node_modules` directory and is not a Deno project that runs without one, and `cache` warns when a persisted extraction cache exists but would not be reused, naming the reason and the on-disk size. A project with no cache yet passes. A project without installed dependencies therefore reports the aggregate `warn` where it previously reported `pass`, still exiting 0. `dependencies` reuses the existing `project` category; `cache` introduces a new `checks[].category` value, `cache`, so a consumer pinning the exact category set should widen it by one.
...
- **`fallow trace-error [FILE|-]` is a new command.** It reads a runtime stack
  ...
  the 1 MiB limit, or a failed analysis exits 2. The command has no MCP tool
  yet.
...
- **The three analysis caches change format together, so the first run after
  upgrading is cold.** The extraction cache moves to version 288, the duplication
  token cache to 13, and the graph cache to 49. Entries in the extraction cache
  ...
  Older blobs are refused on version and rebuilt; nothing about the cache
  location, the `cache` config field, or `--no-cache` changes. Two consequences
  are worth naming: a warm tree copied with `cp -Rp` to a sibling path is reused
  instead of reparsed, and on Windows, where the inode change time is
  unavailable, the metadata-only fast path is disabled and every entry is read
