...
fn resolve_with_graph(
    ...
    let resolved_frames: Vec<ErrorTraceFrame> = frames
        ...
        .enumerate()
        .map(|(index, frame)| resolve_frame(graph, root, index, frame, &mut line_offsets))
        .collect();
...
fn summary_reason(counts: &ErrorTraceCounts) -> String {
    use std::fmt::Write as _;

    if counts.frames == 0 {
        return if counts.unparsed_lines == 0 {
            "no stack frames in the input".to_string()
        } else {
            format!(
                "no stack frames recognised in {} non-blank input {}",
    ...
    if counts.unparsed_lines > 0 {
        let _ = write!(
            reason,
            "; {} input {} not recognised as a frame",
            counts.unparsed_lines,
            plural(counts.unparsed_lines, "line", "lines")
        );
    }
...
fn resolve_frame(
    ...
    let (origin, outcome) = classify_and_look_up(graph, root, &frame, line_offsets);
    let RawFrame {
        raw,
        function,
        is_constructor,
...
fn classify_and_look_up(
    ...
    let module_indexes: Vec<usize> = graph
        ...
        .iter()
        .enumerate()
        .filter(|(_, module)| path_matches(&module.path, root, path))
        .map(|(index, _)| index)
        .collect();
    ...
    (
        FrameOrigin::InProject,
        look_up(graph, root, &module_indexes, identifier, line_offsets),
    )
}
...
fn look_up(
    ...
    let candidates: Vec<ErrorTraceCandidate> = located
        ...
        .map(|candidate| resolve_candidate_line(graph, candidate, line_offsets))
        .collect();

    let (resolution, reason) = match total {
        0 => (
...
fn resolve_candidate_line(
    ...
    let line = line_offsets
        ...
        .or_insert_with(|| {
            graph
                .modules
                .get(candidate.file_id.0 as usize)
