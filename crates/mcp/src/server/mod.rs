...
impl FallowMcp {
    ...
    /// Shortest import path from one module to another, hop by hop with each hop's import line and whether the hop is type-only. Unreachable answers `reachable: false`; the same module answers `reachable: true`. Both report `hops: 0`, so branch on `reachable`, not the count. Use trace_file for one file's direct neighbours.
    #[tool(annotations(read_only_hint = true, open_world_hint = true))]
